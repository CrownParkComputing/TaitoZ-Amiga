...
void YM_DELTAT_ADPCM_Write(YM_DELTAT *D, int r, int v){
    ...
    case 0x09: case 0x0a:
        ...
        /* step = delta * freqbase, freqbase = (chipclock/144)/output_rate (MAME).
         * The old FP-free build pinned freqbase at 1.0, which only holds when
         * output_rate == clock/144 (55.5kHz) -- at 11-22kHz Paula rates Delta-T
         * speech played several times too SLOW.  fm.c's YM2610 init fills
         * step_scale10 with the integer freqbase (.10 fixed point). */
        D->step = D->step_scale10 ? ((D->delta * D->step_scale10) >> 10) : D->delta;
        break;
    case 0x0b:
