...
local pal=nil
for tag,p in pairs(manager.machine.palettes) do pal=p; print("PALETTE tag="..tag.." entries="..tostring(p.entries)) break end
local n=0
emu.register_frame_done(function() n=n+1
  ...
  if n==700 and pal then
    for _,i in ipairs({3472,3473,3474,3475,3476}) do
      local c=pal:pen_color(i)
      print(string.format("MAME pal[%d]=0x%06x  R=%d G=%d B=%d",i,c,(c>>16)&0xff,(c>>8)&0xff,c&0xff))
    end
