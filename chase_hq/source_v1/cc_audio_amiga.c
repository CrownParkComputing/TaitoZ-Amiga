/* cc_audio_amiga.c -- Paula playback for Chase H.Q. sound, CONTINUOUS RING model.
 *
...
/* Audio-cost split for the HUD (EClock ticks; cc_rtg_main reads + zeroes):
 * Z = Z80 driver execution, Y = YM synth render, F = ring flush/copy.
 * Decomposes the HUD's A number so optimisation aims at the real cost. */
ULONG cc_prof_z80, cc_prof_synth, cc_prof_flush;
static ULONG aud_t(void){
    struct EClockVal ev;
    if(!TimerBase) return 0;
    ReadEClock(&ev);
    return ev.ev_lo;
}
static ULONG aud_dt(ULONG b, ULONG a){ ULONG d = b - a; return ((LONG)d < 0) ? 0 : d; }
...
static void ring_render(unsigned long n){
    ...
    while(n){
        ...
        t3 = aud_t();
        cc_prof_z80   += aud_dt(t1, t0);
        cc_prof_synth += aud_dt(t2, t1);
        cc_prof_flush += aud_dt(t3, t2);
        p_wrote += chunk;
        n       -= chunk;
    }
}
