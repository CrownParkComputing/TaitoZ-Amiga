...
/* EI IS caught here (unlike the Galaxian-lineage copies): the Chase HQ sound
 * driver's interrupt-enable window is ~2 instructions (ei; jp loop; di ...),
 * so sampling z_int only at slice boundaries missed most YM timer INTs
 * (measured 31/s accepted vs 61/s fired).  Catching EI lets run_cycles
 * deliver a pending INT at the exact enable point like real hardware --
 * and makes the Z80 underclock safe (acceptance no longer depends on where
 * slice ends land).  DI stays uncaught (pure overhead). */
/* #define Z80_CATCH_DI */
#define Z80_CATCH_EI
/*      
