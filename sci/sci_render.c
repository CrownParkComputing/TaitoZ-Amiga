 ...
 * pens; the RTG/host presenter expands S.C.I.'s XBGR555 palette words. */
#include "sci_state.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
static void draw_road(void){
    ...
    for(int y=0;y<H;y++){
        ...
        /* Keep the road generator's internal per-pixel priority only while
         * choosing between road A/B.  MAME's priority bitmap receives the
         * scanline-level road mask passed to tc0150rod::draw: 1 below the
         * switch line, 2 above it. */
...
/* ---- sprites (ported from sci_draw_sprites_16x8) ---- */
/* Decode one full 16-pen source row of a 16x8 4-plane sprite tile. Replaces the
 * old per-pixel spr_pix16 (4 plane reads + shifts PER PIXEL); the row is decoded
 * once per chunk row and cached across repeated rows when zoomed. Byte layout is
 * identical to MAME's 16x8 OBJ decode: 8 bytes at code*64+sy*8, planes
 * interleaved p*2+(x>>3). */
static inline void spr_row_decode(const uint8_t *gfx,unsigned code,int sy,uint8_t row[16]){
    unsigned base=(code*64u + (unsigned)sy*8u)&0x1fffff;
...
/* Sprite pen DECODE CACHE (St Dragon pattern, lazy): each of the 32768 16x8
 * tiles per gfx bank decodes once into 128 pen bytes; the draw loop then reads
 ...
 * code&0x7fff is exact: code*64 wraps the 2MB bank at 32768 (matches the
 * &0x1fffff masking in spr_row_decode). */
static uint8_t *spr_cache[2];
static uint8_t  spr_cache_valid[2][32768/8];
...
static const uint8_t *spr_cache_row(const uint8_t *gfx,unsigned code,int sy){
    ...
    if(!cache){
        ...
        cache = spr_cache[bank] = (uint8_t*)cc_big_alloc(32768u*128u);
        if(!cache) return 0;
    }
    code &= 0x7fff;
    if(!(spr_cache_valid[bank][code>>3] & (1u<<(code&7)))){
        uint8_t *dst = cache + code*128u;
        for(int r=0;r<8;r++) spr_row_decode(gfx,code,r,dst+r*16);
        spr_cache_valid[bank][code>>3] |= (uint8_t)(1u<<(code&7));
    }
    return cache + code*128u + (unsigned)sy*16u;
}
                              ...
                              int flipx,int flipy,int curx,int cury,int zx,int zy,uint8_t pmask)
{
    ...
    for(int dy=dy0;dy<dy1;dy++,sy_acc+=sy_step){
        int oy=cury+dy;
        int sy=sy_acc>>16; if(sy>7)sy=7; if(flipy)sy=7-sy;
        if(sy!=last_sy){
        ...
        for(int dx=dx0;dx<dx1;dx++,sx_acc+=sx_step){
            ...
            int pidx=dx-dx0;
            uint8_t pri=pr[pidx];
            if((pri & pmask) == 0) o[pidx]=color*16+pix;
            pr[pidx]=31;
        }
    }
}
...
static void draw_sprites(uint16_t*img,int stride){
    ...
    unsigned frame = ((cc_bus_read16(0, 0xc08000) >> 8) & 1);
    int start = frame ? 0 : 0x800;  /* word offset; S.C.I. flips two 0x1000-byte pages */
    for(int offs=start+0x800-4; offs>=start; offs-=4){
        #define SW(n) ((cc_spr[(offs+n)*2]<<8)|cc_spr[(offs+n)*2+1])
        int d0=SW(0),d1=SW(1),d2=SW(2),d3=SW(3);
        int zoomy=(d0&0x7e00)>>9, y=d0&0x1ff;
        int priority=(d1&0x8000)>>15, color=(d1&0x7f80)>>7, zoomx=d1&0x3f;
        int fy=(d2&0x8000)>>15, fx=(d2&0x4000)>>14, x=d2&0x1ff;
        int tile=d3&0x1fff;
        #undef SW
        if(!tile) continue;
        zoomx+=1; zoomy+=1; y+=y_offs+(64-zoomy);
        if(x>0x140)x-=0x200; if(y>0x140)y-=0x200;
        uint8_t pmask = primasks[priority];
        unsigned mo=(unsigned)tile<<5;
        for(int ch=0; ch<32; ch++){
            int k=ch%4, j=ch/4;
            int spx=fx?(3-k):k, spy=fy?(7-j):j;
...
void cc_render_frame(uint16_t *out, int stride){
    ...
    for(int y=0;y<CC_H;y++){
        ...
        /* road overlay: pen + scanline-level road priority mask (MAME semantics) */
        { const uint16_t *rl=road_layer+y*CC_W;
          uint8_t road_pri = (uint8_t)((y > road_priority_switch_line) ? 2 : 1);
          for(int x=0;x<CC_W;x++){ uint16_t rv=rl[x]; if(!(rv&0x8000)){ o[x]=rv&0xfff; pl[x]=road_pri; } } }
