...
int main(int argc,char**argv){
    ...
    const char *dswa_env = getenv("SCI_DSWA");
    int coin_frame = getenv("SCI_COIN_FRAME") ? atoi(getenv("SCI_COIN_FRAME")) : 120;
    int start_frame = getenv("SCI_START_FRAME") ? atoi(getenv("SCI_START_FRAME")) : 200;
    int gas_frame = getenv("SCI_GAS_FRAME") ? atoi(getenv("SCI_GAS_FRAME")) : 230;
    int pulse_frames = getenv("SCI_PULSE_FRAMES") ? atoi(getenv("SCI_PULSE_FRAMES")) : 10;
    ...
    ld("data/audiocpu.bin",cc_rom_audio,0x20000);
    ld("data/adpcma.bin",cc_adpcma,0x180000);
    ld("data/adpcmb.bin",cc_adpcmb,0x80000);

    if(dswa_env) cc_set_inputs(0xff,0xff,(uint8_t)strtoul(dswa_env,0,0),0xff,0);
    cc_machine_init();
    cc_audio_init();
    ...
    for(int i=0;i<frames;i++){
        if(getenv("CC_COIN")){                 /* S.C.I. upright digital cabinet raw inputs. */
            uint8_t in0 = 0xff, in1 = 0xff;
            if(i >= coin_frame && i < coin_frame + pulse_frames) in0 &= (uint8_t)~0x04;
        ...
        if(getenv("SCI_SUBTRACE")){
            int lo = atoi(getenv("SCI_SUBTRACE")); int hi = lo + 200;
            if(i >= lo && i < hi){
    ...
    if(getenv("CC_AUDIODUMP")){ extern int cc_dbg_ymw,cc_dbg_sytc,cc_dbg_nmi,cc_dbg_slaver;
        ...
        printf("RAW: ymPeak=%d FMkeyon=%d ADPCMAkey=%d DeltaT=%d  iff1_EVER=%d INT_accepted=%d/%d\n",cc_dbg_ymraw,cc_dbg_keyon,cc_dbg_adpcma,cc_dbg_deltat,cc_dbg_iffever,cc_dbg_intok,cc_dbg_irq); }
    if(getenv("SCI_COVERAGE")){
        cc_machine_coverage_dump(getenv("SCI_COVERAGE"));
    }
    ...
    if(getenv("CC_SPRDUMP")){
        for(int o=0;o<0x800/2;o+=4){
            ...
            int tile=d3&0x1fff; if(!tile) continue;
            printf("  spr[%d] d=%04x,%04x,%04x,%04x tile=%04x x=%d y=%d zoomx=%d zoomy=%d color=%02x pri=%d fx=%d fy=%d\n",
                o/4, d0,d1,d2,d3, tile, d2&0x1ff, d0&0x1ff,
                (d1&0x3f)+1, ((d0&0x7e00)>>9)+1, (d1&0x7f80)>>7,
                (d1&0x8000)?1:0, (d2&0x4000)?1:0, (d2&0x8000)?1:0);
        }
    }
